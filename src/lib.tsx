...
export function Kicker({ children, tone = "gold" }: { children: ReactNode; tone?: "gold" | "green" }) {
  return (
    <p
      className={`font-cond text-sm font-semibold uppercase tracking-[0.28em] ${
        tone === "green" ? "text-pitch-600" : "text-gold-500"
      }`}
    >
      <span
        className={`mr-3 inline-block h-2 w-2 translate-y-[-1px] ${
          tone === "green" ? "bg-gold-600" : "bg-gold-500"
        }`}
      />
      {children}
    </p>
        ...
        <Reveal delay={170}>
          <p
            className={`mt-5 max-w-2xl text-lg leading-relaxed ${align === "center" ? "mx-auto" : ""} ${
              onDark ? "text-bone-50/70" : "text-pitch-900/70"
            }`}
          >
            {sub}
          </p>
...
export function PageHead({
  ...
  return (
    <section className="relative overflow-hidden bg-pitch-950 pb-16 pt-40 lg:pb-20 lg:pt-48">
      <div className="pitch-lines absolute inset-0 opacity-70" aria-hidden="true" />
      <div
        className="absolute -left-32 top-0 h-96 w-96 rounded-full bg-pitch-500/20 blur-[120px]"
        aria-hidden="true"
      />
      <div
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        <p className="flex items-center gap-2 font-cond text-sm font-semibold uppercase tracking-[0.24em] text-bone-50/45">
          ...
          <span className="text-bone-50/70">{crumb}</span>
        </p>
        <Reveal>
          <Kicker>{kicker}</Kicker>
        </Reveal>
        <h1 className="mt-4 font-display text-5xl uppercase leading-[0.92] text-bone-50 sm:text-6xl lg:text-7xl">
        ...
        {sub && (
          <Reveal delay={200}>
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-bone-50/70">{sub}</p>
          </Reveal>
        )}
