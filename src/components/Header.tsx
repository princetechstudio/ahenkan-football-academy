...
export default function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      {/* utility bar */}
      <div
        className={`hidden overflow-hidden bg-pitch-950 text-bone-50/75 transition-all duration-500 md:block ${
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-4 py-2 sm:px-6">
          ...
          <div className="flex items-center gap-6 font-cond text-sm tracking-wide">
            ...
            <a
              ...
              className="text-bone-50/75 transition-colors hover:text-gold-400"
            >
      ...
      <div
        className={`border-b transition-all duration-500 ${
          scrolled
            ...
            : "border-transparent bg-pitch-950/60 backdrop-blur-sm"
        }`}
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-4 py-3 sm:px-6">
          <a href="#top" className="group flex items-center gap-3">
            <img
              ...
              className="h-12 w-12 rounded-full border-2 border-gold-500/70 object-cover shadow-[0_0_0_3px_rgba(5,19,12,1)] transition-transform duration-300 group-hover:scale-105"
            />
            <span className="leading-none">
              ...
              <span className="mt-1 block font-cond text-[10px] font-semibold uppercase tracking-[0.32em] text-bone-50/55">
          ...
          <nav className="hidden items-center gap-7 lg:flex" aria-label="Primary">
            {NAV.map((n) => (
              <a
                key={n.href}
                href={n.href}
                className="nav-link font-cond text-base font-semibold uppercase tracking-[0.16em] text-bone-50/85 transition-colors hover:text-bone-50"
              >
                {n.label}
              </a>
            ))}
        ...
        >
          <div className="overflow-hidden">
            <nav className="space-y-1 border-t border-bone-50/10 px-4 py-4 sm:px-6" aria-label="Mobile">
              {NAV.map((n, i) => (
                <a
                  key={n.href}
                  href={n.href}
                  onClick={() => setOpen(false)}
                  className="flex items-center justify-between border-b border-bone-50/8 px-2 py-3 font-cond text-xl font-semibold uppercase tracking-[0.14em] text-bone-50/85 transition-colors hover:text-gold-400"
