      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-14 px-4 sm:px-6 lg:grid-cols-12">
        <div className="lg:col-span-6">
          <div className="lg:sticky lg:top-36">
            ...
            />
            <Reveal delay={220}>
              <div className="mt-8 grid grid-cols-1 gap-px border-2 border-ink/10 bg-ink/10 sm:grid-cols-2">
                <div className="bg-lav-50 p-6">
                  <p className="font-cond text-xs font-bold uppercase tracking-[0.24em] text-royal-500">Our Mission</p>
                  <p className="mt-2 font-display text-xl uppercase leading-snug">
                    To become one of the best academies in the world
                  </p>
                </div>
                <div className="bg-royal-900 p-6 text-paper">
            ...
            <Reveal delay={300}>
              <figure className="group relative mt-8 overflow-hidden border-2 border-ink/10">
                <img
                  src={IMG.pitch}
        ...
        {/* the triad */}
        <div className="lg:col-span-6">
          <p className="font-cond text-sm font-bold uppercase tracking-[0.26em] text-ink/50">
          ...
          <div className="mt-6 space-y-6">
            {VALUES.map((v, i) => (
              <Reveal key={v.word} delay={i * 110}>
                <article
                  className={`group border-2 border-ink/10 bg-lav-50 p-7 transition-all duration-300 hover:-translate-y-1 hover:border-royal-500 hover:bg-paper hover:shadow-[0_18px_44px_rgba(126,1,183,0.15)] ${
                    i === 1 ? "lg:translate-x-8" : ""
                  }`}
                >
                  <div className="flex items-baseline gap-5">
                    <span className="font-display text-6xl leading-none text-royal-200 transition-colors duration-300 group-hover:text-gold-500">
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <h3 className="font-display text-3xl uppercase tracking-wide text-royal-700 sm:text-4xl">
                      {v.word}
                    </h3>
                  </div>
                  <p className="mt-4 leading-relaxed text-ink/70">{v.desc}</p>
                </article>
              </Reveal>
            ))}
          </div>
          ...
          {TIMELINE.map((t, i) => (
            <Reveal key={t.title} delay={i * 100}>
              <div className="group relative pb-12 pl-8 last:pb-0 sm:pl-12">
                ...
                <p className="mt-3 max-w-2xl leading-relaxed text-paper/70">{t.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
  ...
  return (
    <section className="relative bg-lav-50 py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="mt-14 grid grid-cols-1 gap-10 md:grid-cols-2">
          {LEADERSHIP.map((l, i) => (
            <Reveal key={l.role} delay={i * 130} variant={i === 0 ? "left" : "right"}>
              <article className="group relative border-2 border-ink/10 bg-paper transition-all duration-300 hover:-translate-y-1.5 hover:border-royal-500 hover:shadow-[0_24px_60px_rgba(126,1,183,0.16)]">
        ...
        <Reveal delay={200}>
          <div className="mt-14 flex flex-wrap items-center justify-between gap-6 border-2 border-royal-500 bg-royal-500 px-7 py-6">
